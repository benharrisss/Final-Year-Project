#!/bin/bash
#SBATCH --job-name=roberta_baseline
...
OUTDIR=/users/sc23bh2/baseline_testing/results/roberta_results
MODELDIR=/scratch/sc23bh2/models/roberta-base
...
srun python "$SCRIPT" \
  ...
  --train-csv "$TRAIN" \
  --test-csv "$TEST" \
  --output-csv "$OUTDIR/roberta_70_30_preds.csv" \
  --training-rounds 3 \
  --batch-size 8 \
  --learning-rate 2e-5 \
  --max-tokens 512